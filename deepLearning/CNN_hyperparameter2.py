...
def build_model(hp):
    model = Sequential()
    model.add(Conv1D(filters=hp.Int('filters', min_value=16, max_value=356, step=16),
                     kernel_size=hp.Int(
    ...
    model.add(Flatten())
    model.add(Dense(units=hp.Int('dense_units', min_value=8, max_value=356, step=16),
                    activation='relu'))
    model.add(Dense(3, activation='softmax'))
    model.compile(optimizer=hp.Choice('optimizer', ['adam', 'nadam', 'rmsprop', 'sgd']),
